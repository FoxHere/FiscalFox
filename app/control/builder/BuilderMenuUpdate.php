...
class BuilderMenuUpdate extends TPage
{
    ...
    public function onAskUpdate($param = null)
    {
        try 
        {
            ...
            {
                if (!file_exists('menu-public-dist.xml')) 
                {
                    throw new Exception(_t('File not found') . ':<br> menu-public-dist.xml');
                }
                $action = new TAction([$this, 'onUpdateMenu'], ['menu_public'=>1]);   
            }
    ...
    public static function onUpdateMenu($param)
    {
        try 
        {
            ...
            {
                if (!file_exists('menu-public.xml') or !is_writable('menu-public.xml')) 
                {
                    throw new Exception("Error Processing Request", 1);
                }
                
                $menuPath = 'menu-public.xml';
                copy('menu-public-dist.xml', $menuPath);

                $newMenu = BuilderPageService::getPublicMenu();  
            }
            
            $menu    = new BuilderMenuWriter($menuPath);
            $submoduleAdded = [];
